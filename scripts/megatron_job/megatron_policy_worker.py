...
class MegatronPolicyWorker(AbstractPolicyWorker, ColocatablePolicyInterface):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        if pt_checkpoint_exists:
            ...
            # --- BEGIN MONKEY PATCH FOR GEMMA 3 + MEGATRON ---
            # Gemma 3 stores its LayerNorm weights mathematically as 'w_raw' such that 
            # the forward pass multiplies by (w_raw + 1.0). Megatron-Core does not, and
            # simply multiplies by the raw weight (0.0). This collapses the entire model
            # activation topology to 0.0. To fix this, we monkey-patch AutoBridge to
            # add 1.0 to all norm layer weights before they're copied to Megatron.
            ...
            def patched_from_hf_pretrained(cls, hf_model_name_or_path, *args, **kwargs):
                ...
                print(f"Intercepting AutoBridge for {hf_model_name_or_path} to apply Gemma-3 corrections...")
                ...
                if getattr(bridge_instance, "hf_pretrained", None) is not None:
                    ...
                    print("Applying +1.0 offset to all RMSNorm weights for Megatron compatibility...")
                    
                    # Unpack the Megatron-Bridge lazy proxy object if necessary
                    actual_model = getattr(hf_model, "model", hf_model)
                    
                    for name, module in actual_model.named_modules():
                        if "norm" in name.lower() or "layernorm" in name.lower():
                            if hasattr(module, "weight") and module.weight is not None:
                                with torch.no_grad():
                                    if module.weight.abs().mean().item() < 0.5:
                                        module.weight.add_(1.0)
                                        # (Silenced the individual print statement to avoid log spam across thousands of tensors)
                                    
                    # --- STATE DICT INTERCEPTOR TO FIX MEGATRON SCHEMA MISMATCH ---
                    # Megatron-Core expects the model to be natively laid out as a Gemma3VLModel
                    # but NeMo wraps it in AutoModelForCausalLM (prepending "model.").
                    # Additionally, Megatron unexpectedly expects the inner language layer to be
                    # wrapped in ".model" (i.e. language_model.model.layers) whereas modern HF is NOT.
                    original_state_dict = actual_model.state_dict
                    def _patched_state_dict(*args, **sd_kwargs):
                        sd = original_state_dict(*args, **sd_kwargs)
                        new_sd = {}
                        for k, v in sd.items():
                            # Remove the 'model.' wrapper imposed by AutoModelForCausalLM
                            if k.startswith("model."):
                                k = k.replace("model.", "", 1)
                                
                            # Inject '.model.' into language_model for Megatron's expectation
                            if k.startswith("language_model.layers") or k.startswith("language_model.norm") or k.startswith("language_model.embed_tokens"):
                                k = k.replace("language_model.", "language_model.model.", 1)
                                
                            new_sd[k] = v
                            
                        # Also bind embeddings to the output layer since transformers fails to construct it consistently
                        if "language_model.lm_head.weight" not in new_sd and "language_model.model.embed_tokens.weight" in new_sd:
                            new_sd["language_model.lm_head.weight"] = new_sd["language_model.model.embed_tokens.weight"]
                            
                        return new_sd
                        
                    actual_model.state_dict = _patched_state_dict
        ...
        if 'original_from_hf' in locals():
            AutoBridge.from_hf_pretrained = original_from_hf
        ...
        # Force vocabulary size modulo to 1 so padded vocab size perfectly matches Gemma's native 262,208 limits.
        model_cfg.make_vocab_size_divisible_by = 1
    ...
    def generate(
        ...
        inference_wrapper_config = InferenceWrapperConfig(
            ...
            inference_max_seq_length=self.cfg["max_total_sequence_length"],  # MUST track the full sequence length, not just generated tokens
        ...
        dynamic_context = DynamicInferenceContext(
            ...
            max_sequence_length=self.cfg["max_total_sequence_length"], # Prevent request_to_kv_block_ids PyTorch tensor slice overflow
