...
retry_cp manifests/02b_megatron/grpo-gemma3-1b-it-megatron.yaml $RAY_HEAD_POD:/workspace/grpo-gemma3-1b-it-megatron.yaml
retry_cp scripts/megatron_job/download_model_1b.py $RAY_HEAD_POD:/workspace/download_model_1b.py
...
DETACHED_CMD="nohup bash -c '
    ...
    /opt/nemo_rl_venv/bin/python /workspace/download_model_1b.py
    ...
    ray job submit --working-dir /workspace --no-wait -- bash -c \"export HF_TOKEN=$HF_TOKEN && export HF_HOME=/data/huggingface && cd /opt/nemo-rl && /opt/nemo_rl_venv/bin/python examples/run_grpo_math.py --config /workspace/grpo-gemma3-1b-it-megatron.yaml\"
' > /data/nemo-rl-logs/detached_launch_1b.log 2>&1 &"
...
echo "kubectl exec -it $RAY_HEAD_POD -- tail -f /data/nemo-rl-logs/detached_launch_1b.log"
